$(document).ready(function () {
    ...
    var table = $('#clientreview_table').DataTable({
        processing: true,
        serverSide: true,
        ajax: window.APP_URLS.getclientReviewdata,
        order: [[0, 'desc']],
        columns: [
            { data: 'id', name: 'id' },
            { data: 'name', name: 'name' },
    ...
    // delete milestone
    $(document).on('click', '.delete_client_review', function () {
        let id = $(this).data('id');

        var url = window.APP_URLS.deleteclientReview.replace(':id' , id);
...
function validateAndPreviewImage() {
        ...
        const errorSpan = document.getElementById("span_image");
        const previewImg = document.getElementById("preview_image");
        const bannerId = document.getElementById("span_clientsay_image_id")?.value;
