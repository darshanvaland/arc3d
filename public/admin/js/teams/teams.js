$(document).ready(function () {

    $('#description').summernote({
        placeholder: 'Enter Description here...',
        height: 100,
        toolbar: [
            ['style', ['style']],
            ['font', ['bold', 'italic', 'underline', 'clear']],
            ['fontname', ['fontname']],
            ['color', ['color']],
            ['para', ['ul', 'ol', 'paragraph']],
            ['height', ['height']],
            ['insert', ['link', 'picture', 'hr']],
            ['view', ['fullscreen', 'codeview']],
            ['help', ['help']]
        ] 
    });  
 
    $(".modal").on("hidden.bs.modal", function () {
        $("#teams_form")[0].reset()
        $("#preview_teams_image").hide();
        $('#description').summernote('code', '');  
        $(".error").html(''); // Hide the preview image after successful save
        
    });
    console.log("ready");
    var imagePath = window.APP_URLS.image_path;
    // console.log("Image Path: " + imagePath);
    var table = $('#teamstable').DataTable({
        processing: true,
        serverSide: true, 
        ajax: window.APP_URLS.teams_get_data,   
        columns: [
            { data: 'id', name: 'id' },
            { data: 'name', name: 'name' },
            { data: 'designation', name: 'designation' },
            {
                data: 'image',
                name: 'image', 
                orderable: false, 
                searchable: false,
                render: function (data, type, row) {
                    if (data) {
                        return '<img src="' + imagePath +'/' +  data + '" alt="' + row.image + '" width="60" height="60">';
                    } else {
                        return '<span class="text-muted">No Image</span>';
                    }
                }
            },
            { data: 'status', name: 'status' },
            { data: 'action', name: 'action', orderable: false, searchable: false }
        ]
    });


    $("#Saveteams").on('click', function () {
        // if (valid_teams_status() & validateAndPreviewImage() & valid_teams() & valid_designation() & valid_description()) {
            if (valid_teams_status() & validateAndPreviewImage()) {
            var teamsId = $("#teams_id").val();
            
            var url = window.APP_URLS.teamsStore;
            
            // var url = window.APP_URLS.teamsStore;
            var formData = new FormData($("#teams_form")[0]);
            $.ajax({ 
                type: "POST",
                url: url,
                data: formData,
                contentType: false,
                processData: false,
                success: function (response, textStatus, jqXHR) {
                    $("#message-pop-up").removeClass('alert-success alert-warning')
                    if (response.result) {
                        $('#teams_modal').modal('hide');
                        $("#teams_form")[0].reset()
                        $("#teams_id").val();
                        $("#message-pop-up").attr('style', 'display:block')
                        $("#message-pop-up").addClass('alert-success')
                        $("#success-message").html(response.message);
                        $("#preview_teams_image").hide(); // Hide the preview image after successful save
                        $("#color_value").val(''); // Clear the color value
                        setTimeout(() => {
                            $("#message-pop-up").attr('style', 'display:none')
                        }, 3000);
                        table.draw();
                    } else {
                        $('#teams_modal').modal('hide');
                        $("#teams_form")[0].reset()
                        $("#message-pop-up").attr('style', 'display:block')
                        $("#message-pop-up").addClass('alert-warning')
                        $("#success-message").html(response.message);
                        setTimeout(() => {
                            $("#message-pop-up").attr('style', 'display:none')
                        }, 3000);
                    }
                },
                error: function (jqXHR, textStatus, errorThrown) {
                    console.table(jqXHR)
                }
            });
        } else {
            return false
        } 

    })

    $(document).on('click', '.edit_teams', function () {
        var teams_id = $(this).data('id');
        var url = window.APP_URLS.teams_edit_data.replace(':id', teams_id);
        $.ajax({
            type: "GET",
            url: url,
            success: function (response, textStatus, jqXHR) {
                if (response.result) {
                    $("input[name='teams_status']").prop('checked', false);
                    $('#teams_modal').modal('show');
                    $("#teams_id").val(response.data.id);
                    $("#name").val(response.data.name);
                    $("#alt_tag").val(response.data.alt_tag);
                    $("#designation").val(response.data.designation);
                    $("#description").summernote('code', response.data.description);
                    $("input[name='teams_status'][value='" + response.data.status + "']").prop('checked', true);
                    // ✅ Show existing banner image if available

                    if (response.data.image) {
                        $("#preview_teams_image").show(); // full URL or relative path
                        $("#preview_teams_image")
                            .attr('src', window.APP_URLS.image_path + '/' + response.data.image);

                    }
                } else { 
                    $("#message-pop-up").attr('style', 'display:block')
                    $("#message-pop-up").addClass('alert-warning')
                    $("#success-message").html(response.message);
                    setTimeout(() => {
                        $("#message-pop-up").attr('style', 'display:none')
                    }, 3000);
                }
            },
            error: function (jqXHR, textStatus, errorThrown) {
                console.table(jqXHR)
            }
        });
    })
 
    $(document).on('click', '.delete_teams', function () {
        teams_id = $(this).data('id');
        var url = window.APP_URLS.teams_delete_data.replace(':id', teams_id);
        var confrim_delete = confirm("Are You Sure want To Delete?");
        if (confrim_delete) {
            $.ajax({
                type: "Delete",
                url: url,
                headers: {
                    'X-CSRF-TOKEN': window.APP_URLS.csrfToken
                },
                success: function (response, textStatus, jqXHR) {
                    if (response.result) {
                        $("#message-pop-up").attr('style', 'display:block')
                        $("#message-pop-up").addClass('alert-success')
                        $("#success-message").html(response.message);
                        setTimeout(() => {
                            $("#message-pop-up").attr('style', 'display:none')
                        }, 3000);
                        table.draw();
                    } else {
                        $("#message-pop-up").attr('style', 'display:block')
                        $("#message-pop-up").addClass('alert-warning')
                        $("#success-message").html(response.message);
                        setTimeout(() => {
                            $("#message-pop-up").attr('style', 'display:none')
                        }, 3000);
                    }
                },
                error: function (jqXHR, textStatus, errorThrown) {
                    console.table(jqXHR)
                }
            });
        }

    })


    function valid_teams(){
        if($("#name").val() == ''){
            $("#span_name").text("Enter Name This Filed is Required.")
            return false
        }else{
            $("#span_name").text("")
            return true;
        }
    }
    function valid_designation(){
        if($("#designation").val() == ''){
            $("#span_designation").text("Enter Designation This Filed is Required.")
            return false
        }else{
            $("#span_designation").text("")
            return true;
        }
    }
    function valid_description(){
        if($("#description").val() == ''){
            $("#span_desc").text("Enter Description This Filed is Required.")
            return false
        }else{
            $("#span_desc").text("")
            return true;
        }
    }

    function valid_teams_status() {
        if ($("input[name='teams_status']:checked").length === 0) {
            $("#span_teams_status").text("Please select a status.");
            return false;
        } else {
            $("#span_teams_status").text("");
            return true;
        }
    }

    // Run validation immediately when user clicks radio
    $("input[name='teams_status']").on("change", function () {
        valid_teams_status();
    });
});
function validateAndPreviewImage() {
    const input = document.getElementById("teams_image");
    const file = input.files[0];
    const errorSpan = document.getElementById("span_teams_image");
    const previewImg = document.getElementById("preview_teams_image");
    const bannerId = document.getElementById("teams_id")?.value;

    // Allow update without new image
    if (bannerId && !file) {
        errorSpan.innerText = "";
        return true;
    }

    if (!file) {
        errorSpan.innerText = "Image is required.";
        previewImg.style.display = "none";
        return false;
    }

    if (!file.type.startsWith("image/")) {
        errorSpan.innerText = "Only image files are allowed.";
        input.value = ""; // reset input
        previewImg.style.display = "none";
        return false;
    }

    // Show image preview
    const reader = new FileReader();
    reader.onload = function (e) {
        previewImg.src = e.target.result;
        previewImg.style.display = "block";
        errorSpan.innerText = "";
    }
    reader.readAsDataURL(file);

    return true;
}

// 🔹 Auto-validate when user selects a file
document.getElementById("teams_image").addEventListener("change", function () {
    validateAndPreviewImage();
});
